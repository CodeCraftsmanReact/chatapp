...
const ChatWindow = () => {
    ...
    // example chat data
    //sort by dateTime
    //replace to and from with UUID
    const chat = [
        {
            id: 1,
            type: 'message',
            from: "Joseph Kertz",
            to: "Shannon Matusek-Steele",
            dateTime: "11:46",
            status: "Delivered",
        ...
        },
        {
            ...
            dateTime: "11:48",
            status: "Delivered",
            message: "Thanks for the feedback. Our users will really depend on your improvements."
        },
        {
            id: 3,
            type: 'message',
    ...
    // const chat=[];
  return (
    <div className='w-[80%] min-h-[100%] pb-20 relative mt-4 mx-auto bg-white/50 border border-white border-solid rounded p-8'>
        {
           chat.length > 0 &&  chat.map((message) => {
                if(!message.reaction){
                    return( message.from === "Joseph Kertz" ? 
                    <ChatBubbleOut message={message}/> :
                    <ChatBubbleIn message={message}/>)
                }
    ...
    return(
        ...
        <form class="mx-auto">   
            ...
            <div class="relative">
                ...
                <button type="submit" class="text-white absolute end-2.5 bottom-2.5  focus:ring-4 focus:outline-none font-medium  text-sm px-4 py-2  bg-blue-600  hover:bg-blue-700  focus:ring-blue-800"><Icon icon="tabler:send"/></button>
            </div>
        </form>
        ...
        <div class="flex items-start gap-2.5 mb-4">
            ...
            <div class="flex flex-col w-full max-w-[320px] leading-1.5 p-4 border-gray-200 rounded-e-xl rounded-es-xl bg-gray-700/80">
                <div class="flex items-center space-x-2 rtl:space-x-reverse">
                    <span class="text-sm font-semibold   text-white">{message.from}</span>
    ...
    return(
        <div class="flex items-start justify-end gap-2.5 mb-4">
            <div class="flex flex-col w-full max-w-[320px] leading-1.5 p-4 border-gray-200 rounded-s-xl rounded-es-xl bg-gray-700/80">
                <div class="flex items-center space-x-2 rtl:space-x-reverse">
                    <span class="text-sm font-semibold   text-white">{message.from}</span>
                    <span class="text-sm font-normal   text-gray-400">{message.dateTime}</span>
                </div>
